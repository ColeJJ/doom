...
- Steuerung ueber die Variable `+idea-build-dependencies` (Default `t`).
  ...
  schnellen Pfad, wenn `~/.m2` bereits aktuell ist (dann nur `compile exec:java`).
...
bzw. `mc`). Der `mvn exec:java`-Weg kompiliert selbst (`compile exec:java`).
